...
const victoireOrdinateur = () => {
  message.textContent = 'Perdu bouuuh';
  scoreOrdinateur.textContent++;
};

const victoireJoueur = () => {
  message.textContent = 'Bien joué cousin, continues comme ça';
  scoreJoueur.textContent++;
};

const preprarerNouvelleManche = () => {
  btnJoueur.forEach((btn) => {
...
nextBtn.addEventListener('click', preprarerNouvelleManche);
